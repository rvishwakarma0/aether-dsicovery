...
import { PRESETS, getPrefixedChip } from '../utils/chips';
...
export default function DiscoveryScreen({
  ...
  const handleToggleChip = (chip: string) => {
    const prefixed = getPrefixedChip(chip);
    setSelectedChips(prev => {
      ...
      if (!isSelected) {
        setQuery(currentQuery => {
          ...
          if (!trimmed) {
            return prefixed.slice(0, 250);
          }
          const lowerQuery = currentQuery.toLowerCase();
          const hasPlain = lowerQuery.includes(chip.toLowerCase());
          const hasPrefixed = lowerQuery.includes(prefixed.toLowerCase());
          if (!hasPlain && !hasPrefixed) {
            const separator = trimmed.endsWith(',') ? ' ' : ', ';
            const newQuery = `${trimmed}${separator}${prefixed}`;
            return newQuery.slice(0, 250);
          }
          return currentQuery;
        });
        return [...prev, chip];
        ...
        setQuery(currentQuery => {
          ...
          const escapedPlain = chip.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
          const patternPrefixed = `\\s*,?\\s*${escapedPrefixed}\\s*,?\\s*|\\s*${escapedPrefixed}\\s*`;
          const patternPlain = `\\s*,?\\s*${escapedPlain}\\s*,?\\s*|\\s*${escapedPlain}\\s*`;
          const regexPrefixed = new RegExp(patternPrefixed, 'gi');
          const regexPlain = new RegExp(patternPlain, 'gi');
          let newQuery = currentQuery.replace(regexPrefixed, ', ').replace(regexPlain, ', ').trim();
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 border-r border-slate-200">
        ...
        <form onSubmit={handleSearchSubmit} className="space-y-4">
          <div className="space-y-1">
            <div className="relative">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" aria-hidden="true" />
              <label htmlFor="destination-search-input" className="sr-only">Search destinations</label>
              <input
              ...
              <button
                type="submit"
                disabled={isLoading}
                aria-label={isLoading ? "Discovering destinations..." : "Discover destinations"}
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap gap-1.5 items-center" role="group" aria-label="Filter by where">
            ...
            <div className="flex flex-wrap gap-1.5 items-center" role="group" aria-label="Filter by month">
            ...
            <div className="flex flex-wrap gap-1.5 items-center" role="group" aria-label="Filter by vibe">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        {errorMessage && (
          <div role="alert" className="p-3.5 bg-red-50 border border-red-100 text-red-700 rounded-xl text-xs flex items-start gap-2">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 px-4 border border-dashed border-slate-200 rounded-xl bg-white">
            <MapPin className="w-8 h-8 text-slate-400 mx-auto mb-2" aria-hidden="true" />
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        aria-label={`Select ${dest.name}, ${dest.hook}`}
                        className={`w-full text-left p-4 rounded-xl border transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 ${
                          isSelected
                      ...
                      >
                        <div className="flex justify-between items-start gap-2 mb-1.5">
                          <span className="font-display font-bold text-base tracking-tight leading-tight block">
                            {dest.name}
                          </span>
                          <span
                            aria-hidden="true"
